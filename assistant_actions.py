        ...
        for order in orders:
            for j, item in enumerate(order[0]):
                if item == itemNumber and (order[1][j] > quantity or quantity <= 0):
                    ...
                    order[1][j] -= quantity
                    if quantity == 1:
                        text_to_speech(translate("1 order of " + name + " was removed from your order.", language), language)
                    else:
...
def completeOrder(orders, transactions, menuItems, language):
    ...
    while True:
        ...
        if answer == "yes":
            ...
            text_to_speech(translate("Sorry, I don't understand that response. Is your order correct?", language), language)
            language, response = speech_to_text(language)
            response = translate(response, "en")
